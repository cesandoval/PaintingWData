...
function extractZip(zipFile, callback){
  ...
  extract(zipFile, {dir: filePath}, function(err){
    if(err){  
      ...
      console.log('successs unzipping')
...
function getEPSG(file, callback) {
  ...
  if (layer.srs.getAttrValue("AUTHORITY",1)==null){
    ...
    // let sridQuery = "SELECT srid, srtext, proj4text FROM public.spatial_ref_sys AS t WHERE t.srtext ILIKE '%"+layer.srs.toWKT()+"%'"
    // sequelize.query(sridQuery).spread(function(results, metadata){
    //   console.log(results, 12121212121212)
    // })

    if (layer.srs.validate() == null) {
      let d_srs = gdal.SpatialReference.fromEPSGA(4326)
    ...
    } else if (typeof srsParse == 'undefined') {
