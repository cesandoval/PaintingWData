...
var dbURL = 'postgresql://postgres:postgrespass@localhost:5432/PaintingWithData_Riyadh'
...
function pushDataLayerTransform(file, epsg, newName, req, layer, callback) {
    ...
    while(feature = layer.features.next()) {
        ...
        if (geom != null){ 
            ...
            datalayersToInsert.push(newDatalayer);
    ...
    var queryString = format('INSERT INTO public."Datalayers" (layername, "userId", "datafileId", epsg, "userLayerName", description, location, properties, rasterval, "rasterProperty", "createdAt", "updatedAt") VALUES %L', datalayersToInsert);
    console.log(queryString);
    exit();
    connection.query(queryString).spread(function(results, metadata) {
        console.log("done!");
    });
    // pg.connect(dbURL, function(err, client, done) {
    //             if(err) throw err;
    //             var query = new QueryStream(queryString, [1000000])
