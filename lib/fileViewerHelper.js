...
function loadSimplifiedDatalayers(datafileId, req, callback) {
    Model.Datafile.findOne({
        ...
        var geomQuery = `SELECT geom.geometry FROM public."Datalayers" AS geom WHERE geom."datafileId"= '` + datafileId +`';`
        connection.query(geomQuery).spread(function(datalayers){
            console.log(datalayers[0])
            callback(null, [datalayers, datafile.bbox, datafile.centroid])
        })
        // datafiles.bbox, datafiles.centroid 
    })
