...
function pushDataLayerTransform(file, epsg, newName, req, layer, geomType, callback) {
    // Creates a couple of spatial reference objects, one for the files projection, and a second for WGS84
    var s_srs = gdal.SpatialReference.fromEPSGA(epsg),
        d_srs = gdal.SpatialReference.fromEPSGA(4326);
    
    // Creates a spatial transformation between spatial references 
 ...
 * TODO: eliminate this argument, we no longer need to only save a single property
...
function transformPoints(layer, d_srs, rasterProperty) {
    ...
    while(feature = layer.features.next()) {
        ...
        var currPt = {
            type: 'Feature',
            /**
             * TODO: The properties should include all of the properties in the geometry
             * "properties": {
             *      "prop0": "value0",
             *      "prop1": { "this": "that" }
             * }
             * You can probably get all the properties by doing: feature.fields.toObject()
             */
            properties: {
...
function processPoints(geoJSON, bbox) {
    ...
    /**
     * TODO: run this function for all the properties, and save the different properties as new properties of the geometries 
     * "properties": {
     *      "prop0": "value0",
     *      "prop1": { "this": "that" }
     * }
    ...
    // Return all the geometries computed
    // TODO: return the geometries with all the properties instead of a single property
