export default class Exporter {
    ...
    static parseGeometries(pixels, layerName, remap=false, translation=false) {
        ...
        if (remap != false && translation != false) {
            ...
            var rgbColor = ["rgb(",remap(color.r),",", remap(color.g),",", remap(color.b),")"].join("");
        ...
        for (let i = 0, j = 0; i < geomTranslation.length; i = i + 3, j++){
            ...
            if (x!=0 && y!=0) {
                ...
                if (remap != false && translation != false) {
                    var currCircle = ['<circle id="', j, '" r="', size, '" cx="', x+translation[0], '" cy="', y+translation[1], '" fill="', 
                    rgbColor, '" fill-opacity="', opacity, '" stroke-width="0"></circle>'].join("");

                    allGeometries = allGeometries.concat(currCircle);
                } else {
                    var currPoint = {type: "Feature",
                                    geometry: {type: "Point", 
                                            coordinates: [x,y]},
                                    properties: {
                                        value: size,
    ...
    static exportSVG(geometries, translation, centroid, bounds) {
        ...
        let allSVG = [['<svg width="', bounds[0], 'px" height="', bounds[1], 'px" viewBox="0 0 ',bounds[0], ' ', bounds[1],'">'].join("")];
        for (let i in layers) {
            allSVG = allSVG.concat(this.parseGeometries(geometries[layers[i]], layers[i], remap, translation));
        }   
