    ...
    $.widget("custom.progressWidget", {
            ...
            if ((isLayerUrl || isVoxelUrl) && urlHasUploads) {
                try {
                    ...
                    if (isLayerUrl){
                        ...
                        this.state.queryIds.shapes.add(layerInfo);
                    }
                    else {
                        ...
                        this.state.queryIds.voxels[hash] = new Set(dataLayerIds);
                    }
        ...
        */
        _queryController: function(){
            ...
            var arrOfQueries = Array.from(this.state.queryIds.shapes);
            $.each(this.state.queryIds.voxels, function(hash, dataLayerIds){
                arrOfQueries.push(hash + '$' + dataLayerIds.size);
            });
        ...
        */
        _updateArrays: function(){
            ...
            this.progress; // Reminder: this shows an array of 5-length arrays: [id, jobName, num, den, pollType]
            for (var job in this.progress){
                var newJob = true;
                for (var candJob in this.state.jobs){
        ...
        /**
        * Given a job (5-length array etc.), the job is removed from this.state.queryIds.
        */
        _removeFromQueryIds: function(job){
            if (job[4] == "voxels"){ // JSON vs Set. Lol.
                delete this.state.queryIds[job[4]][job[0]];
            }
            if (job[4] == "shapes"){
                this.state.queryIds[job[4]].delete(job[0]);
            }
        },
        ...
        */
        _updateWidgetMenu: function(){
            this.state.jobs.forEach(job => {
                var [id, jobName, numerator, denominator, type] = job,
        ...
        */
        _defaultState: function(){
            return {
                ...
                queryIds: {
                    shapes: new Set([]), // A set of ID's.
                    voxels: {},  // A JSON, mapping a voxel's hash property to a set of its corresponding data layer IDs.
                },
                jobs: [] // Each element of the array is of the form: [id, jobName, numerator, denominator, type (voxel or shape)]
            };
        },
    });
}
